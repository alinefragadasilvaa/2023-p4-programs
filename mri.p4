...
header ipv4_option_t {
    bit<1> copyFlag;
    bit<2> optClass;
    bit<5> option; // tipo 31 indica cebaçalho mri
    bit<8> optionLength;
}

header mri_t {
    bit<16>  count; // campo que deve conter informações sobre antigos saltos
}

header switch_t { 
    switchID_t  swid;
    qdepth_t    qdepth; // profundidade da fila
}
...
struct parser_metadata_t {
    bit<16>  remaining; // utilizado no estado recursivo do parser para determinar a parada
}
...
struct headers {
    ...
    mri_t              mri;
    switch_t[MAX_HOPS] swtraces; // o pacote pode ter passado por mais de um switch, se for o caso, contém mais de uma cabeçalho para analisar
}
...
parser MyParser(packet_in packet,
    ...
    state parse_swtrace {
        ...
        *   - Otherwise, transition to parse_swtrace.
        */
        packet.extract(hdr.swtraces.next); // estado recursivo para analisar todos as informações dos switches de saltos anteriores
...
control MyEgress(inout headers hdr,
    ...
    action add_swtrace(switchID_t swid) {
        ...
        */
        ...
        hdr.swtraces[0].setValid(); // validando por conta da especificação da versão da linguagem
